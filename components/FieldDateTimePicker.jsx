...
const FieldDateTimePicker = ({ title, value, mode, handleChangeDateTime, icon, otherStyles, ...props }) => {
  ...
  return (
    <View className={`space-y-2 ${otherStyles}`}>
      ...
      <TouchableOpacity 
        className='border-2 border-black-200 w-full h-16 px-4 bg-primary rounded-2xl focus:border-secondary items-center flex-row'
        activeOpacity={0.7}
      ...
      {mode === 'date' && (
        <>
          <Text
            className={`flex-1 ${value ? 'text-black' : 'text-black-200'} font-psemibold text-base`}
          >
